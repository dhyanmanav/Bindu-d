import { Home, Search, Library, PlusSquare, Heart, Briefcase, FileText, Code, Award, GraduationCap, Mail, X } from 'lucide-react';
    ...
    <>
      ...
      `}>
        ...
        <nav className="px-3">
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-4 px-3 py-2 rounded transition-colors ${currentView === item.id
                ? 'text-white'
        ...
        <nav className="px-3 mt-4 flex-1 overflow-y-auto">
          <button
            onClick={() => window.open('/Bindu_D.pdf')}
